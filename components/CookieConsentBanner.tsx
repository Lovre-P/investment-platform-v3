...
const CookieConsentBanner: React.FC<CookieConsentBannerProps> = ({ onConsentChange }) => {
  ...
  const handleAcceptAll = async () => {
    setIsLoading(true);
    try {
      await cookieConsentService.acceptAll(); // Added await
      const preferences = cookieConsentService.getPreferences(); // This is sync
      onConsentChange?.(preferences);
      setIsVisible(false);
    } catch (error) {
      console.error('Error accepting cookies in CookieConsentBanner:', error);
  ...
  const handleRejectAll = async () => {
    setIsLoading(true);
    try {
      await cookieConsentService.rejectAll(); // Added await
      const preferences = cookieConsentService.getPreferences(); // This is sync
      onConsentChange?.(preferences);
      setIsVisible(false);
    } catch (error) {
      console.error('Error rejecting cookies in CookieConsentBanner:', error);
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleSavePreferences = async () => {
    ...
    try {
      ...
      console.error('Error saving preferences in CookieConsentBanner:', error);
    } finally {
      setIsLoading(false);
    }
  };
